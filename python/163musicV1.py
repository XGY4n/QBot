...
musicPath = "./resource/music/" #\\resource
...
def download_song(url, SongName, SongType):
    ...
    if r.status_code == 200:
        bot_print("\033[1;32;40m{}\033[0m".format("Response<{}>".format(r.status_code)))
        bot_print("Downloading---"+ SongName + "." + SongType)
        f = open(musicPath + SongName + "." + SongType, 'wb')
        f.write(r.content)
        f.close()
        bot_print( "End "+ SongName)
    else :
        bot_print("\033[1;32;40m{}\033[0m".format("Response<{}>".format(r.status_code)))
        return "Response<{}>".format(r.status_code)
...
def open_url(RawName):
    search =RawName #sys.argv[1]TwistedNerve
