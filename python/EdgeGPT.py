...
async def main(row, args):
    ...
    #print("Bot:")
    if args.no_stream:
        print(( await bot.ask(prompt=question, conversation_style=args.style,wss_link=args.wss_link))
              ["item"]["messages"][1]["adaptiveCards"][0]["body"][0]["text"] + "11")
        #print(["item"]["messages"][1]["adaptiveCards"][0]["body"][0]["text"])
    else:
        if args.rich:
            wrote = 0
            md = Markdown("")
...
def binginterface(row):
    ...
    parser.add_argument(
        "--proxy", help="Proxy URL (e.g. socks5://127.0.0.1:7890)", type=str
    )
    parser.add_argument(
    ...
    try:
        ans = asyncio.run(main(row, args))
    except Exception as e:
        return str(e)
    #print(ans)
    #ans += "!!!!!code change from https://github.com/acheong08/EdgeGPT license: https://unlicense.org!!!!!"
    return ans
